...
public class Solution {
    public static void main(String[] args) throws Exception {
        ArrayList <Integer> list = new ArrayList<>();// создать список общий, крат3, крат2, остальные
        ArrayList <Integer> list3 = new ArrayList<>();
        ArrayList <Integer> list2 = new ArrayList<>();
        ArrayList <Integer> listOther = new ArrayList<>();
    BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
    //заполняем общий список
            for (int i = 0; i < 20 ; i++) list.add(Integer.parseInt(reader.readLine()));

            for (int i = 0; i < 20; i++) {//заполняем остальные списки
            if (list.get(i)% 3 == 0) list3.add(list.get(i));//крат3
            if (list.get(i)% 2 == 0) list2.add(list.get(i));//крат2
